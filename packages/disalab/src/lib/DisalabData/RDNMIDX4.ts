...
export class RDNMIDX4 {
  ...
  constructor(doctor: string, surname: string, regdate: unknown, labno: string, server?: DisaServer) {
    this.#server = server;
    this.DOCTOR = doctor;
  ...
  static async All(where: string, server: DisaServer): Promise<RDNMIDX4[]> {
    const DB_DRIVER = server.config.database.driver;
    const DB_URI = server.config.database.connection_string;
    const results: RDNMIDX4[] = [];

    if (DB_DRIVER === "mssql") {
      try {
        const sql = `SELECT [DOCTOR] ,[SURNAME] ,[REGDATE] ,[LABNO] FROM [DisalabData].[dbo].[RDNMIDX4] ${!Core.IsEmpty(where) ? where : ""}`;

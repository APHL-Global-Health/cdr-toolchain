...
- **Concurrency model.** Every phase runs in parallel — DISA fetch, v1 check, and POST. There is no mutex; connections come from a registry keyed by connection string, so DISA and v1 hold one pool each. `--concurrency 5` therefore means five whole labs in flight, not five HTTP calls. The ceiling is mssql's default `max: 10` connections per pool — past that, raise it with `Max Pool Size=N` in the connection string rather than changing code. Note the CLI defaults `--concurrency` to 1 but `scripts/push-to-ce.sh` defaults it to 4. See [CDR_TOOLCHAIN.md](CDR_TOOLCHAIN.md) for the full detail.
